...
public class CacheRow {
	...
	private boolean dirty; // Dirty bit to notify of an update in write-back policy, 0 for clean, 1 for
							// dirty.
	...
	private int LRU; // Least Recently Used, the last used set in a specific index will be replaced
						// on eviction. Only for Set-Associatice.
	...
	public String toString() {
		...
		if (this.data == null)
			return String.format(
					"ERROR: Null data found in RowIndex %d, Set %d. Time of requests too close together.\n", this.index,
					this.set);
		for (int d : this.data) {
			dataArray.append('[');
			dataArray.append(d);
		...
		return String.format("%b\t| %b\t| %d \t| %d \t| %d \t| %d \t| %s \t\n", this.valid, this.dirty, this.set,
				this.index, this.LRU, this.tag, dataArray);
	}
}
